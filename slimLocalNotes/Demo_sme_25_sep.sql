...
CREATE VIEW l5min_wiki as select * from wiki_kafka_hive 
where `__timestamp` >  1000 * to_unix_timestamp(CURRENT_TIMESTAMP - interval '5' MINUTES) ; 
...
select distinct `user`  as `user` , page as `first_name` , floor(1000/rand())%100 as age ,  
case when (rand() > 0.5) then 'M' else 'F' end as gender , comment  
...
-- Will be nice to add the following example as part of the demos 
...
-- Add kerberos local ticket cache
-- alter table kafka_truck_geo_events SET TBLPROPERTIES ("kafka.consumer.sasl.jaas.config"="com.sun.security.auth.module.Krb5LoginModule required useTicketCache=true;");
-- Notes to my self

-- This Demo is using the following Nodes as Brokers 

--server.1=cn105-10.l42scl.hortonworks.com:2888:3888
--server.2=cn106-10.l42scl.hortonworks.com:2888:3888
--server.3=cn107-10.l42scl.hortonworks.com:2888:3888

-- Running kafka 

-- /mnt/nfs0/kafka_kafka_2.12-1.0.1/bin/kafka-server-start.sh /mnt/nfs0/kafka_kafka_2.12-1.0.1/config/server_107.properties

-- Running Wiki Ticker on CN105 using Screen -> wiki